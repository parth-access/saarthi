...
export default async function handler(req: any, res: any) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      const rules = snapshot.docs.map((doc: any) => ({ id: doc.id, ...doc.data() }));
    ...
    if (req.method === 'POST') {
      ...
      if (!rule.therapistId || rule.dayOfWeek === undefined || !rule.startTime || !rule.endTime) {
        return res.status(400).json({ success: false, error: 'Missing rule parameters' });
      }

      await db.collection('availability_rules').add({
        ...rule,
        createdAt: new Date()
      });
    ...
    if (req.method === 'DELETE') {
      const { id } = req.query;
      if (!id) return res.status(400).json({ success: false, error: 'Rule ID required' });
