...
import { ArrowLeft, BookOpen, Wind, Search, Heart, PlayCircle, Lock, Play } from "lucide-react";
import { useRouter } from "next/navigation";

export default function Resources() {
  const router = useRouter();
  const [activeTab, setActiveTab] = useState("all");
  const [playingExercise, setPlayingExercise] = useState<string | null>(null);
  ...
  return (
    <div className="min-h-screen pt-24 pb-24 bg-[#FFFBE7]">
      <div className="container mx-auto px-6 max-w-6xl">
        ...
        {/* Filters */}
        <div className="flex flex-wrap items-center gap-4 mb-10 border-b border-primary/5 pb-4">
          {categories.map((c) => (
            <button
              key={c.id}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  {breathingExercises.map((ex) => (
                    ...
                    >
                      ...
                      <p className="text-primary/60 text-sm leading-relaxed mb-6">{ex.desc}</p>
              ...
              <section>
                ...
                <div className="bg-white rounded-3xl border border-primary/10 p-8 sm:p-10 shadow-sm relative overflow-hidden">
                   ...
                   <div className="relative z-10 max-w-xl">
                      <p className="text-xs font-bold tracking-widest text-[#E6A520] uppercase mb-3">Daily Prompt</p>
                      <h3 className="text-2xl sm:text-3xl font-serif text-primary mb-4 leading-tight">
                        "What is one thing you can let go of today that isn't serving you?"
                      </h3>
                      ...
                      <button className="flex items-center justify-center gap-2 px-6 py-3 bg-primary text-white rounded-full text-sm font-medium hover:bg-primary/90 transition-colors shadow-sm w-fit">
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="bg-white rounded-3xl border border-primary/10 p-8 shadow-sm">
              ...
              <button className="w-full mt-6 py-3 border border-primary/10 rounded-2xl text-sm font-medium text-primary hover:bg-[#FFFBE7] transition-colors">
                View All Articles
              </button>
            </div>

            <div className="bg-primary text-[#FFFBE7] rounded-3xl p-8 relative overflow-hidden">
               ...
               <button onClick={() => router.push("/therapists")} className="w-full py-3 bg-[#E6A520] text-primary font-medium text-sm rounded-xl hover:bg-[#c48b1a] transition-colors">
                 Find a Therapist
               </button>
            </div>
          </div>
