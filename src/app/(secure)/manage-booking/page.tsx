...
function ManageBookingPage() {
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setError((err instanceof Error ? err.message : String(err)) || "Invalid or expired booking link.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSlotSelect = async (time: string) => {
    ...
    try {
      ...
      setError((err instanceof Error ? err.message : String(err)) || "Failed to reschedule. Please try again.");
  ...
  return (
    <React.Suspense fallback={
      <div className="pt-32 pb-20 flex flex-col items-center justify-center min-h-[60vh] bg-[#FFFBE7] font-sans">
        <Loader2 className="w-10 h-10 text-primary animate-spin mb-4" />
        <p className="font-serif italic text-primary/60">Loading session manager...</p>
      </div>
    }>
      <ManageBookingPage />
    </React.Suspense>
